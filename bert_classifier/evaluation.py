...
from cross_dataset import SentencePairConcatDataset, SentencePairTestConcatDataset
...
# Perform model evaluation in terms by averaging accuracies across tasks.
def model_eval_multitask_cross(sentiment_dataloader,
                         paraphrase_dataloader,
                         sts_dataloader,
                         model, device):
    model.eval()  # switch to eval model, will turn off randomness like dropout
...
def model_eval_test_multitask(sentiment_dataloader,
    ...
    with torch.no_grad():
        ...
        para_sent_ids = []
        # Evaluate paraphrase detection.
        for step, batch in enumerate(tqdm(paraphrase_dataloader, desc=f'eval test', disable=TQDM_DISABLE)):
            (b_ids1, b_mask1,
             b_ids2, b_mask2,
        ...
        # Evaluate semantic textual similarity.
        for step, batch in enumerate(tqdm(sts_dataloader, desc=f'eval test', disable=TQDM_DISABLE)):
            (b_ids1, b_mask1,
             b_ids2, b_mask2,
             b_sent_ids) = (batch['token_ids_1'], batch['attention_mask_1'],
                          batch['token_ids_2'], batch['attention_mask_2'],
        ...
        # Evaluate sentiment classification.
        for step, batch in enumerate(tqdm(sentiment_dataloader, desc=f'eval test', disable=TQDM_DISABLE)):
...
# Perform model evaluation in terms by averaging accuracies across tasks.
def model_eval_test_multitask_cross(sentiment_dataloader,
                         paraphrase_dataloader,
                         sts_dataloader,
                         model, device):
    model.eval()  # switch to eval model, will turn off randomness like dropout
...
def test_model_multitask_cross(args, model, device):
        sst_test_data, num_labels,para_test_data, sts_test_data = \
            load_multitask_data(args.sst_test,args.para_test, args.sts_test, split='test')
        ...
        para_test_data = SentencePairTestConcatDataset(para_test_data, args)
        ...
        sts_test_data = SentencePairTestConcatDataset(sts_test_data, args)
        ...
        dev_paraphrase_accuracy, dev_para_y_pred, dev_para_sent_ids, \
            dev_sentiment_accuracy,dev_sst_y_pred, dev_sst_sent_ids, dev_sts_corr, \
            dev_sts_y_pred, dev_sts_sent_ids = model_eval_multitask_cross(sst_dev_dataloader,
                                                                    para_dev_dataloader,
                                                                    sts_dev_dataloader, model, device)

        test_para_y_pred, test_para_sent_ids, test_sst_y_pred, \
            test_sst_sent_ids, test_sts_y_pred, test_sts_sent_ids = \
                model_eval_test_multitask_cross(sst_test_dataloader,
                                          para_test_dataloader,
                                          sts_test_dataloader, model, device)
