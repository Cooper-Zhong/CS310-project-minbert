...
class BertSelfAttention(nn.Module):
    ...
    def attention(self, key, query, value, attention_mask):
        ...
        # TODO
        # q, k, v: [bs, num_attention_heads, seq_len, attention_head_size]
        bs, _, seq_len = key.shape[:3]
        # multiply key and value and divide by sqrt(dim_k)
        attn_val = torch.div(query.matmul(key.transpose(2, 3)), math.sqrt(self.attention_head_size))
        ...
        attn_output = attn_val.matmul(value)
        # concat multi-heads and recover the original shape
        attn_output = attn_output.transpose(1, 2)
        attn_output = attn_output.reshape(bs, seq_len, self.all_head_size)
        return attn_output
